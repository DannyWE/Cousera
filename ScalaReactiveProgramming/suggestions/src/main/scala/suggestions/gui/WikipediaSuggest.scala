...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms
      .concatRecovered(wikiSuggestResponseStream(_).timedOut(1))
    ...
    val selections: Observable[String] = button.clicks.map(
     {
      button =>
         Observable{suggestionList.selection.items: _*}
     }
    ).concat

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.sanitized.concatRecovered(wikiPageResponseStream(_).timedOut((1)))

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      _ match {
        case Success(body) => editorpane.text = body
        case Failure(error) => status.text = error.getMessage
      }
    }
