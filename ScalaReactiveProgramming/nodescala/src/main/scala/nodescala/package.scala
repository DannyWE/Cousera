...
//      Future.sequence{fs}
      ...
      val suc = Promise[List[T]]()
//      suc.success(Nil)
      fs.foldRight(Future.successful((List.empty[T]))) {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      fs map {x : Future[T] => x onComplete { p.tryComplete(_) }   }
...
//      f
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      val token = tokenSource.cancellationToken
      f(token)
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] =
    {
      ...
      f onComplete {
        case _ => promise.complete(Try(cont(f)))
      }
      promise.future
    }
     ...
     *  The resulting future contains a value returned by `cont`.
     */
    def continue[S](cont: Try[T] => S): Future[S] = continueWith(x => cont(x.value.get))
  }
  ...
  object CancellationTokenSource {
    ...
    def apply(): CancellationTokenSource = {
      ...
      new CancellationTokenSource {
        ...
        def unsubscribe() : Unit =  promise.complete(Success(Unit))
      }
